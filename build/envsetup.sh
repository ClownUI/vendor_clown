...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^clown_") ; then
        CLOWN_BUILD=$(echo -n $1 | sed -e 's/^clown_//g')
    else
        CLOWN_BUILD=
    fi
    export CLOWN_BUILD
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/clown-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function clownremote()
{
    ...
    local CLOWN_USER=$(git config --get review.review.clownui.org.username)
    if [ -z "$CLOWN_USER" ]
    then
        git remote add clown ssh://review.clownui.org:29418/$PFX$PROJECT
    else
        git remote add clown ssh://$CLOWN_USER@review.clownui.org:29418/$PFX$PROJECT
    fi
...
function clowngerrit() {
    ...
    local user=`git config --get review.review.clownui.org.username`
...
function clownrebase() {
    ...
    git fetch "http://review.clownui.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CLOWN_FIXUP_COMMON_OUT ]; then
