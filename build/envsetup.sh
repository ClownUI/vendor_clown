function __print_ClownUI_functions_help() {
cat <<EOF
Additional Project __print_ClownUI_functions_help functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- ClownUIgerrit:   A Git wrapper that fetches/pushes patch from/to ClownUI Gerrit Review.
- ClownUIrebase:   Rebase a Gerrit change and push it again.
- ClownUIremote:   Add git remote for ClownUI Gerrit Review.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/clown/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch clown_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ClownUI-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function clownremote()
{
    ...
    git remote rm clown 2> /dev/null
    ...
    local CLOWN_USER=$(git config --get review.review.clown.org.username)
    if [ -z "$CLOWN_USER" ]
    then
        git remote add clown ssh://review.clown.org:29418/$PFX$PROJECT
    else
        git remote add clown ssh://$CLOWN_USER@review.clown.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'clown' created"
}
...
function makerecipe() {
    ...
    then
        pwd
        clownremote
        git push clown HEAD:refs/heads/'$1'
    fi
    '
}

function clowngerrit() {
    ...
    local user=`git config --get review.review.clown.org.username`
...
function clownrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ClownUI Gerrit Rebase Usage: "
        echo "      clownrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.clown.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
